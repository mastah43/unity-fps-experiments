...
namespace Cosmocraft
{
    ...
    public class ZombieController : MonoBehaviour
    {
        ...
        [Tooltip("Walk leg angle maximum")] public double legWalkAngle = 20;
        ...
        private void Walk()
        {
            ...
            */
            ...
            /*
            Logger.Log("walk",$"Walk at time={Time.time}: agent velocity={agent.velocity.magnitude}"
        ...
        private void UpdateWalkMovement(ref WalkMovement movement)
        {
            ...
            var footForwardPassed =
                (long)((movement.StepsWalked - 0.5) / 1L) - (long)((stepsWalkedBefore - 0.5) / 1L) == 1;
            var footDistanceDelta = footForwardPassed
                ? Math.Abs(FootDistanceByLegRotation(legWalkAngleForCurrentSpeed) -
                           FootDistanceByLegRotation(legRotation)) +
                  Math.Abs(FootDistanceByLegRotation(legWalkAngleForCurrentSpeed) -
                           FootDistanceByLegRotation(legRotationBeforeUpdate))
            ...
            // Logger.Log("walk", $"walk update: footDistanceDelta={footDistanceDelta}, agentDistanceDelta={agentDistanceDelta}, stepNowCompleted={footForwardPassed}, legRotation={legRotation}, legRotationBeforeUpdate={legRotationBeforeUpdate}");

        }

        private static double DegreesToRand(double degrees) => degrees / 180d * Math.PI;

        private float FootDistanceByLegRotation(double legRotation) =>
            (float)Math.Abs(Math.Sin(DegreesToRand(legRotation)) * legLength);
